...
( function( RPBChessboard, $ ) {
	...
	function undo() {
		...
		// Update the state of the undo/redo buttons.
		$( '#rpbchessboard-editFENDialog-undo' ).button( 0 === nextActionIndex ? 'disable' : 'enable' );
	...
	function switchInteractionMode( newMode ) {
		...
		if ( 'undefined' === typeof newMode || newMode === oldMode ) {
	...
	RPBChessboard.showEditFENDialog = function( args ) {
		...
		// 'args' is a struct
			...
			dialogCallback = ( 'callback' in args && 'function' === typeof args.callback ) ? args.callback : null;
			dialogOptions = ( 'options' in args && 'object' === typeof args.options ) ? args.options : {};
