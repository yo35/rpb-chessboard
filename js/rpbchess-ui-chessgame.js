...
( function( RPBChess, $, moment ) {
	...
	function formatTitle( title ) {
		return ( null === title || '-' === title ) ? null : title;
	}
 ...
 return '$' + nag;
	...
	function capitalize( text ) {
		return 0 === text.length ? '' : text.charAt( 0 ).toUpperCase() + text.slice( 1 );
	}
 ...
 bodyClass += ' rpbui-chessgame-clickableMoves';
	...
	function buildVariation( widget, variation, isMainVariation, result ) {
		...
		if ( null === variation.comment() && null === variation.first() && null === result ) {
	...
	function buildMove( widget, node, forcePrintMoveNumber ) {
		...
		// Move number
		var printMoveNumber = forcePrintMoveNumber || 'w' === node.moveColor();
		var moveNumberClass = 'rpbui-chessgame-moveNumber' + ( printMoveNumber ? '' : ' rpbui-chessgame-hidden' );
		var moveNumberText  = node.fullMoveNumber() + ( 'w' === node.moveColor() ? '.' : '\u2026' );
 ...
 return $( '#rpbui-chessgame-navigationFrame ' + buttonClass );
	...
	function updateNavigationBoard( widget, move, playTheMove ) {
		if ( move === undefined || null === move || move.hasClass( 'rpbui-chessgame-selectedMove' ) ) {
			return;
		}
	...
	function updateNavigationButton( widget, buttonClass, isVisible ) {
		var button = 'frame' === widget.options.navigationBoard ? $( '#rpbui-chessgame-navigationFrame ' + buttonClass ) : $( buttonClass, widget.element );
		if ( isVisible ) {
			button.show();
		} else {
			button.hide();
	...
	function updateNavigationBoardPosition( widget, move, playTheMove ) {
		...
		navigationBoard.chessboard( 'option', 'squareMarkers', 'undefined' === typeof csl ? '' : csl );
		navigationBoard.chessboard( 'option', 'arrowMarkers', 'undefined' === typeof cal ? '' : cal );
	}
	...
	function retrieveNavigationBoard( widget ) {
		return 'frame' === widget.options.navigationBoard ?
	...
	function updateSelectedMove( widget, move ) {
		var global = 'frame' === widget.options.navigationBoard;
